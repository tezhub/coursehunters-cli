...
const File = (jswget.File = function(path) {
  this.path = path;
...
function parseConfig(argv) {
  ...
  if (configs.url !== undefined) {
    ...
    if (url.length > 1) {
      ...
      configs.path =
        configs.path.indexOf('?') == -1 && configs.url.indexOf('?') !== -1
          ? configs.path + '?'
          : configs.path;
      configs.hostname = configs.hostname.split(':');
      configs.port =
        configs.hostname[1] === undefined
          ? configs.protocol == 'http' ? 80 : 443
          : configs.hostname[1];
  ...
  configs.hostname =
    configs.hostname === undefined ? '127.0.0.1' : configs.hostname;
  configs.port = configs.port === undefined ? 80 : configs.port;
  configs.query =
    configs.query === undefined ? '' : QS.stringify(configs.query);
  configs.path =
    (configs.path === undefined ? '/' : configs.path) +
    (configs.query.length > 0
      ? (configs.path.indexOf('?') == -1 ? '?' : '') + '&' + configs.query
      : '');
  ...
  configs.formdata =
    configs.formdata === undefined ? undefined : configs.formdata;
  configs.cookiefile =
    configs.cookiefile === undefined ? '' : configs.cookiefile;
  configs.cookies = configs.cookies === undefined ? {} : configs.cookies;
  configs.headers = configs.headers === undefined ? {} : configs.headers;
  configs.encoding =
    configs.encoding === undefined ? 'utf-8' : configs.encoding;
  configs.callback =
    configs.callback === undefined ? new Function() : configs.callback;
  configs.onsend =
    configs.onsend === undefined ? new Function() : configs.onsend;
  configs.onsuccess =
    configs.onsuccess === undefined ? new Function() : configs.onsuccess;
  configs.ondata =
    configs.ondata === undefined ? new Function() : configs.ondata;
  configs.onredirect =
    configs.onredirect === undefined ? new Function() : configs.onredirect;
  configs.onerror =
    configs.onerror === undefined ? new Function() : configs.onerror;
  configs.onhead =
    configs.onhead === undefined ? new Function() : configs.onhead;
  configs.onend = configs.onend === undefined ? new Function() : configs.onend;
  configs.onsocket =
    configs.onsocket === undefined ? new Function() : configs.onsocket;
  configs.onconnect =
    configs.onconnect === undefined ? new Function() : configs.onconnect;
  configs.onupgrade =
    configs.onupgrade === undefined ? new Function() : configs.onupgrade;
  configs.oncontinue =
    configs.oncontinue === undefined ? new Function() : configs.oncontinue;
  ...
  configs.downloadmode =
    configs.downloadmode === undefined ? false : configs.downloadmode;
  configs.downloadas =
    configs.downloadas === undefined
      ? configs.downloadmode
        ? configs.path.split('/')[configs.path.split('/').length - 1]
        : configs.downloadas
      : configs.downloadas;
  configs.downloadpath =
    configs.downloadpath === undefined ? './' : configs.downloadpath;
  configs.pipestream =
    configs.pipestream === undefined ? undefined : configs.pipestream;
  ...
  configs._followCount =
    configs._followCount === undefined ? 0 : configs._followCount + 1;
  configs.globalcookies = {};
  configs.cookiename =
    configs.protocol + '://' + configs.hostname + ':' + configs.port;
  configs.globalcookies[configs.cookiename] = {};
  if (typeof configs.cookiefile === 'string') {
    if (
      configs.cookiefile.length > 0 &&
      configs._followCount < configs.maxfollow
    ) {
      try {
  ...
  for (var i in configs.globalcookies[configs.cookiename]) {
    if (typeof configs.globalcookies[configs.cookiename][i] === 'string') {
      ...
      if (configs.globalcookies[configs.cookiename][i].path !== undefined) {
        cookie.push(
          'Path=' + configs.globalcookies[configs.cookiename][i].path
        );
      }
      if (configs.globalcookies[configs.cookiename][i].expires !== undefined) {
        cookie.push(
          'Path=' + configs.globalcookies[configs.cookiename][i].expires
        );
      }
      if (
        configs.globalcookies[configs.cookiename][i]['max-age'] !== undefined
      ) {
        cookie.push(
          'Path=' + configs.globalcookies[configs.cookiename][i]['max-age']
        );
      }
      if (configs.globalcookies[configs.cookiename][i].domain !== undefined) {
        cookie.push(
          'Path=' + configs.globalcookies[configs.cookiename][i].domain
        );
      }
      if (
        configs.globalcookies[configs.cookiename][i].commenturl !== undefined
      ) {
        cookie.push(
          'Path=' + configs.globalcookies[configs.cookiename][i].commenturl
        );
      }
  ...
  if (configs.auth !== undefined) {
    configs.headers.Authorization =
      'Basic ' +
      Buffer.from(configs.auth.username + ':' + configs.auth.password).toString(
        'base64'
      );
  ...
  if (configs.oauth !== undefined) {
    ...
    configs.oauth.signature_method =
      configs.oauth.signature_method === undefined
        ? 'HMAC-SHA1'
        : configs.oauth.signature_method;
    configs.oauth.oauth_token_version =
      configs.oauth.oauth_token_version === undefined
        ? '1.0'
        : configs.oauth.oauth_token_version;
    const oauthData = {
    ...
    var oauthURL =
      ...
      '://' +
      configs.hostname +
      (configs.port.length == 0 ? '' : ':' + configs.port) +
      configs.path;
    ...
    const sig = generateOAuthSignature(
      ...
      oauthURL,
      sigData,
      configs.oauth.consumer_secret,
      configs.oauth.token_secret
    );
    ...
    for (var k in oauthData) {
      oauthHeader +=
        ...
        '="' +
        encodeURIComponent(oauthData[k]) +
        '"';
    }
  ...
  if (configs.formdata !== undefined) {
    configs.method = 'POST';
    configs.headers['Content-Type'] =
      'application/x-www-form-urlencoded; charset=UTF-8';
    for (var i in configs.formdata) {
      if (configs.formdata[i] instanceof File) {
        configs.method = 'POST';
        configs._boundaryKey =
          'WebKitFormBoundary' +
          (Math.random() * 1000000000000000).toString(16);
        configs.headers['Content-Type'] =
          'multipart/form-data; boundary="' + configs._boundaryKey + '"';
        configs._isUploadForm = true;
        break;
      }
    }
    if (configs._isUploadForm) {
      const formbufferarray = [];
      for (var i in configs.formdata) {
        var formdatastr = '';
        if (configs.formdata[i] instanceof File) {
          const filename = configs.formdata[i].path.replace(/.+\//gi, '');
          formdatastr =
            '--' +
            configs._boundaryKey +
            '\r\n' +
            ...
            '"\r\n' +
            'Content-Type: ' +
            configs.formdata[i].mime +
            '\r\n\r\n';
          formbufferarray.push(Buffer.from(formdatastr, 'ascii'));
          formbufferarray.push(
            Buffer.from(configs.formdata[i].contents, 'utf8')
          );
        } else {
          formdatastr =
            '--' +
      ...
      configs.formdata = [Buffer.from(QS.stringify(configs.formdata), 'utf-8')];
    }
  }
  let length = 0;
  for (var i in configs.formdata) {
...
function createRequest(protocol, sendoptions, configs) {
  const request = protocol.request(sendoptions);
  request.on('response', response => {
    ...
    if (responsecookie !== undefined) {
      const responsecookieobject = {};
      const cookiekeyheaders = [
        'path',
        'expires',
        'max-age',
        'secure',
      ...
      for (var i = 0; i < responsecookie.length; i++) {
        const responsecookieattr = responsecookie[i].split(';');
        let namefound = false;
        var responsecookiename;
        responsecookieobject[i] = {};
        for (let n = 0; n < responsecookieattr.length; n++) {
          const attr = responsecookieattr[n].split('=');
          ...
          attr.shift();
          const attrval = attr[0] !== undefined ? attr.join('=') : true;
          if (attrval == true) {
        ...
        configs.globalcookies[configs.cookiename][responsecookiename] =
          responsecookieobject[i];
      }
    }
    ...
    if (typeof configs.cookiefile === 'string') {
      if (configs.cookiefile.length > 0) {
        try {
          Fs.writeFileSync(
            configs.cookiefile,
            JSON.stringify(configs.globalcookies, null, 4)
          );
        } catch (e) {}
      }
    }
    ...
    if (response.statusCode == 301 || response.statusCode == 302) {
      if (
        ...
        sendoptions = createSendOption(protocol, configs);
        const newrequest = createRequest(protocol, sendoptions, configs);
      ...
      if (configs.downstream === undefined) {
        ...
        response.on('end', () => {
          ...
          const file = Fs.createWriteStream(
            configs.downloadpath + configs.downloadas,
            {
              flags: 'a+',
            }
          );
          if (startbyte >= filesize) {
            if (filesize > 0) {
              ...
              configs.onsuccess.apply(configs.scope, [
                file,
                request,
                response,
                configs,
          ...
          const sendoptions = createSendOption(protocol, configs);
          configs.onhead.apply(configs.scope, [
            fstat,
            request,
            response,
            configs,
    ...
    } else if (configs.downstream === undefined) {
    ...
    response.on('end', () => {
      configs.callback.apply(configs.scope, [
        request._response_error,
        resultdata,
      ]);
      configs.onsuccess.apply(configs.scope, [
        resultdata,
  ...
  request.on('connect', (response, socket, head) => {
    configs.onconnect.apply(configs.scope, [
      request,
      response,
      socket,
      head,
  ...
  request.on('upgrade', (response, socket, head) => {
    configs.onupgrade.apply(configs.scope, [
      request,
      response,
      socket,
      head,
  ...
  request.on('continue', (response, socket, head) => {
...
function sendRequest(request, configs) {
  ...
  if (configs.timeout !== undefined) {
    request.setTimeout(configs.timeout, () => {
      request.abort();
    });
  }
  if (configs.formdata !== undefined) {
    if (!Array.isArray(configs.formdata)) {
      configs.formdata = [configs.formdata];
    }
    for (const i in configs.formdata) {
...
function generateOAuthSignature(
  ...
  keys.sort();
  const output = method.toUpperCase() + '&' + encodeURIComponent(url) + '&';
  let params = '';
  keys.forEach(k => {
