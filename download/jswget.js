...
function parseConfig(argv) {
  ...
  if (configs.url !== undefined) {
    ...
    if (url.length > 1) {
      ...
      configs.port =
        configs.hostname[1] === undefined ? (configs.protocol == 'http' ? 80 : 443) : configs.hostname[1];
  ...
  configs.path =
    ...
    (configs.query.length > 0 ? (configs.path.indexOf('?') == -1 ? '?' : '') + '&' + configs.query : '');
  ...
  if (configs.auth !== undefined) {
    configs.headers['Authorization'] =
      'Basic ' + new Buffer(configs.auth.username + ':' + configs.auth.password).toString('base64');
  ...
  if (configs.oauth !== undefined) {
    ...
    configs.oauth.signature_method =
      configs.oauth.signature_method === undefined ? 'HMAC-SHA1' : configs.oauth.signature_method;
    configs.oauth.oauth_token_version =
      configs.oauth.oauth_token_version === undefined ? '1.0' : configs.oauth.oauth_token_version;
    var oauthData = {
    ...
    var sig = generateOAuthSignature(
      configs.method,
      oauthURL,
      sigData,
    ...
    if (configs.downloadmode) {
      ...
      if (configs.downstream === undefined) {
        var filesize = parseInt(
          response.headers['content-length'] === undefined ? 0 : response.headers['content-length']
        );
        ...
        response.on('end', function() {
          ...
          var file = Fs.createWriteStream(configs.downloadpath + configs.downloadas, {
            flags: 'a+',
          });
