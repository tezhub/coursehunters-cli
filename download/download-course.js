...
const download = (links = [], courseId, rootDownloadPath, isResource = false) => {
  const homeDir = os.homedir();
  const configExists = fileExists.sync(`${homeDir}/.coursehunters.json`);
  let config = {};
  if (configExists) {
    config = JSON.parse(fs.readFileSync(`${homeDir}/.coursehunters.json`, 'utf8'));
    const course = config.courses.find(course => course.courseId === courseId);
    const courseIndex = config.courses.findIndex(course => course.courseId === courseId);
    if (!course && !isResource) {
      ...
      fs.writeFileSync(`${homeDir}/.coursehunters.json`, JSON.stringify(config, null, 2));
    }
  } else {
    config.courses = [
      {
  ...
  return new Promise((resolve, reject) => {
    ...
    mkdirp(downloadPath, err => {
      ...
      fs.writeFileSync(`${downloadPath}/data.json`, JSON.stringify(links, null, 2));
      if (urls.length === 0) {
        return resolve('Done');
      }
